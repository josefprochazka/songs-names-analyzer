...
```
~/repos-ubuntu/songs-analyzer-backend
~/repos-ubuntu/songs-analyzer-frontend
```
...
- **Pohybovat se čistě linuxovými příkazy**, ne přes Windows cesty. Např. mezi
  ...
  `cd ../songs-analyzer-backend` (jsou to sourozenecké složky ve
  `repos-ubuntu/`).
...
- Před jakoukoliv prací ověřit `pwd`, že jsem opravdu v
  `.../repos-ubuntu/songs-analyzer-frontend`, nespoléhat na zapamatovanou
  cestu z předchozí session.
